---
title: Fragment 交易平台完全指南：从靓号交易到 Stars 提现
shortTitle: Fragment交易
description: 想要+888匿名号码或短用户名？本文介绍Telegram官方区块链交易平台Fragment，详解用户名拍卖、+888虚拟手机号交易、TON钱包连接、Premium折价购买，以及开发者Telegram Stars星币提现流程。
icon: diamond
category:
  - 进阶教程
tag:
...
head:
  - - meta
    - name: keywords
      content: Fragment,TON,Telegram用户名,Telegram匿名号码,Telegram大会员,TG用户名,TG匿名号码,TG大会员,电报用户名,电报匿名号码,电报大会员,Fragment提现,Telegram Stars提现,星币提现
---
...
```mermaid
graph TD
    A[用户支付 Stars] --> B(Telegram 账号余额)
    B --> C{满 21 天且满足提现额度}
    C -->|是| D[前往 Telegram 创作者/开发者后台]
    C -->|否| E[继续等待或积累]
    D --> F[绑定 Fragment 并完成 KYC]
    F --> G[在 Fragment 发起 Stars -> TON 兑换]
    G --> H[TON 实时打入绑定的 Tonkeeper 钱包]
```
...
- [礼物系统完全指南](./topics/gift/gifts.md) — 用 Fragment 购买的礼物如何升级 NFT 与交易
- [高级隐私保护指南](./topics/privacy-advanced.md) — 用匿名号码切断真实身份关联