---
home: true
icon: home
title: 电报宝典
heroImage: ./assets/icon/logo.svg
heroImageDark: ./assets/icon/logo2.svg
bgImage: ./assets/image/0-light.svg
bgImageDark: ./assets/image/0-dark.svg
bgImageStyle:
  background-attachment: fixed
heroText: 电报宝典
tagline: 专业的 Telegram 电报中文知识库，由 ygjc.cc 倾力打造。
heroFullScreen: true
actions:
  - text: 开始阅读
    icon: lightbulb
    link: ./guide.html
    type: primary
  - text: 访问主站
    icon: link
    link: https://ygjc.cc/

# 下面是页脚配置
footer: Copyright © 2025 <a href="https://github.com/tgnav/tgwiki" target="_blank">TGwiki</a> | Powered by <a href="https://ygjc.cc/" target="_blank">ygjc.cc</a>
---

### 🔥 热门教程快速入口

::: 
[**汉化语言包**](./language.html) &nbsp;&nbsp;|&nbsp;&nbsp; [**解除+86私聊限制**](./spam.html) &nbsp;&nbsp;|&nbsp;&nbsp; [**解除敏感内容限制**](./pornios.html)
:::

::: 
[**收不到验证码？**](./notcomesms.html) &nbsp;&nbsp;|&nbsp;&nbsp; [**账号防封指南**](./banned.html) &nbsp;&nbsp;|&nbsp;&nbsp; [**下载官方客户端**](./guide.html#在开始之前)
:::

<br/>

> 觉得有用？访问主站 [**ygjc.cc**](https://ygjc.cc) 获取更多硬核互联网教程与资源。